...
-   사례로 알아보는 성능 개선 팁(초기 로딩 다이어트, 라이브러리 재사용 등)
    -   성능 개선을 위한 시점
        -   최초 로딩시
        -   다른 마이크로앱으로 최초에 넘어갈 때
        -   마이크로 앱 내에서 특정 UI 와 상호작용할 때
    -   성능 개선을 위한 문제 찾아내기
        -   당장 사용하지 않는 코드 찾아내서 제외하기 (라이브러리 코드 혹은 소스 코드 둘다)
        -   인터렉션할 때 랜더하지 않아도 되는 부분 찾아내서 랜더하지 않게 만들기

### 보안 이슈

-   마이크로 프론트엔드의 보안 이슈 해결

    -   Module Federation 사용과 관련된 보안 이슈

        1. 코드 신뢰성
            - Webpack Module Federation 을 사용할 때 코드 신뢰성 문제는 특히 중요함
            - 예를 들어, 메인 애플리케이션이 외부 팀이 제공하는 로그인 모듈을 Module Federation 을 통해 로드한다고 가정해 보면, 외부 팀의 코드는 사용자 인터페이스의 일부가 되며, 이 코드가 안전하지 않다면 전체 애플리케이션의 보안에 심각한 위협이 될 수 있음
            - 이러한 이유로, Module Federation 을 사용할 때는 외부에서 가져온 모든 코드를 신뢰하기 전에 철저히 검토하고 테스트 해야함
            - 이는 코드 리뷰, 보안 감사, 그리고 자동화된 취약점 스캐닝 도구를 사용하여 실시할 수 있음
            - 또한, 외부 코드를 프로덕션 환경에 배포하기 전에, 개발 및 스테이징 환경에서 충분히 검증하는 것이 중요함
            - 이렇게 함으로써, 잠재적인 보안 위험을 최소화하고 안전한 애플리케이션 운영을 보장할 수 있음
        2. 종속성 관리

            - Webpack Module Federation 을 활용할 때 종속성 관리는 중요한 보안 측면 중 하나임
            - 이 방식에서 여러 애플리케이션 또는 마이크로 프론트엔드가 공유하는 종속성은 보안 취약점을 포함할 수 있으며, 이는 전체 시스템에 영향을 미칠 수 있음
            - 예를 들어, 여러 마이크로 프론트엔드가 공유하는 공통 UI 라이브러리(예: React UI 컴포넌트 라이브러리)가 있다고 가정해보면, 이 라이브러리가 오래된 버전의 lodash 와 같은 외부 패키지에 의존하고 있다면, lodash 에 존재하는 보안 취약점은 모든 마이크로 프론트엔드 애플리케이션에 영향을 미칠 수 있으며, 이러한 종속성은 때때로 업데이트가 누락되거나 간과될 수 있으므로, 정기적인 보안 검토와 업데이트가 필수적임
            - 종속성 관리는 모듈 페더레이션을 사용하는 프로젝트의 안전성을 유지하기 위해 지속적인 관심과 노력이 필요한 영역이고, 자동화된 취약점 스캐닝 / 정기적인 업데이트와 감사 / 최소한의 종속성 원칙 등을 이용해서 적절한 관리를 통해, 보안 위험을 줄이고 프로젝트의 안정성을 향상시킬 수 있음

        3. CORS 설정
            - Webpack Module Federation 을 사용하는 컨텍스트에서 Cross-Origin Resource Sharing(CORS) 설정은 중요한 보안 고려사항이며, CORS 는 웹 페이지가 다른 도메인의 리소스에 접근할 수 있도록 허용하는 메커니즘으로, 잘못 구성되었을 때 보안 문제를 야기할 수 있음
            - 예를 들어, 메인 애플리케이션(A) 이 다른 도메인의 마이크로 프론트엔드 모듈(B) 을 로드한다고 가정해 보면, B 모듈의 서버는 A 애플리케이션의 요청을 수락하도록 CORS 헤더를 설정해야하며, 만약 CORS 정책이 너무 개방적이라면, 악의적인 출처에서도 B 모듈의 리소스에 접근할 수 있어 보안 취약점이 발생할 수 있음
            - CORS 설정은 각 마이크로 프론트엔드 모듈의 서버 구성에 따라 다르게 적용될 수 있으며, 보안을 강화하면서도 필요한 리소스 공유를 가능하게 하는 균형을 찾는 것이 중요하며, 잘못된 CORS 구성은 데이터 유출, CSRF 공격 등 다양한 보안 위험을 초래할 수 있으므로, 신중한 접근이 필요함
            - 적절한 CORS 구성을 위해서는 다음 사항들을 고려해야함
                - 명시적인 출처 지정 : CORS 정책에서는 특정 출처만을 허용하도록 설정하고, Access-Control-Allow-Origin 헤더에 구체적인 도메인을 지정하여, 신뢰할 수 있는 출처만 리소스에 접근하도록 제한함
                - 필요한 메소드만 허용 : Access-Control-Allow-Methods 를 통해 허용되는 HTTP 메소드를 제한을 하면, GET 요청만 허용하고 POST, PUT, DELETE 는 차단할 수 있음
                - 자격 증명 설정 : Access-Control-Allow-Credentials 헤더를 사용하여 쿠키나 인증 토큰 같은 자격 증명을 포함한 요청의 처리 방법을 결정
        4. 버전 호환성
            - Webpack Module Federation 을 사용할 때 버전 호환성 문제는 특히 중요한 고려사항이며, 이는 마이크로 프론트엔드 아키텍처에서 각 애플리케이션 또는 컴포넌트가 독립적으로 개발되고 배포되기 때문에 발생임
            - 예를 들어, 메인 애플리케이션(A) 이 사용하는 리액트 버전이 16.8 이고, 이와 통합되는 다른 마이크로 프론트엔드(B) 가 리액트 17.0 을 사용한다고 가정해보면, A 와 B 가 서로 다른 리액트 버전을 사용하면, 런타임에서 호환성 문제나 충돌이 발생할 수 있으며, 이는 예기치 않은 버그나 오류로 이어질 수 있고, 애플리케이션의 안정성을 저하시킬 수 있음
            - 이러한 문제를 해결하기 위해서는 다음과 같은 접근 방법을 사용할 수 있음
                - 버전 정합성 확인 : 각 마이크로 프론트엔드 컴포넌트가 사용하는 종속성의 버전을 정확하게 확인하고, 가능한 한 호환성 있는 버전을 사용하도록 함
                - 단일 버전 정책: 중요한 라이브러리에 대해 모든 마이크로 프론트엔드가 동일한 버전을 사용하도록하는 정책을 수립하면, 이를 통해 런타임 충돌을 최소화할 수 있음
                - 의존성 공유: Module Federation 설정에서 중요한 라이브러리를 공유 의존성으로 설정하여, 여러 마이크로 프론트엔드 간에 동일한 라이브러리 인스턴스를 사용할 수 있도록 함
            - 버전 호환성 문제는 모듈 페더레이션 환경에서 중요한 보안 및 안정성 이슈를 야기할 수 있으므로, 이에 대한 신중한 계획과 검증이 필요함
        5. 데이터 노출 및 유출
            - 모듈 페더레이션에서 데이터 노출 및 유출 문제는 주로 마이크로 프론트엔드 간의 데이터 공유 및 통합 과정에서 발생하며, 이러한 아키텍처에서, 각 마이크로 프론트엔드는 독립적으로 개발되고 배포되지만, 실행 시 서로의 코드나 데이터를 공유할 수 있으며, 이 과정에서 민감한 데이터가 적절히 보호되지 않으면 보안 위험이 발생할 수 있음.
            - 예를 들어, 메인 애플리케이션(A) 이 다른 마이크로 프론트엔드(B) 로부터 사용자 정보를 로드하는 경우를 생각해보면, 만약 B가 API 를 통해 사용자 데이터를 전송하고 이 데이터가 암호화되지 않은 채로 전송되거나, B 자체에 보안 취약점이 있는 경우, 중간 공격자가 데이터를 가로채거나 노출시킬 수 있음.
            - 데이터에 접근하는 각 마이크로 프론트엔드의 접근 권한을 엄격하게 관리해야하며, 불필요한 데이터 공유를 방지하고, 최소한의 데이터만을 전송하도록 해야함.
            - 이러한 조치들은 모듈 페더레이션 환경에서 데이터 보호를 강화하고, 민감한 정보의 노출 및 유출을 방지하는 데 필수적임

### 팀 운영하기

-   팀을 어떻게 운영해야 할까?(마이크로 프론트엔드와 팀의 운영 구조)

    1. 분산된 팀 구조

        - 분산된 팀 구조의 설정은 각 기능 또는 비즈니스 도메인별로 팀을 구성하는 것을 의미하며, 이는 전통적인 모노리식 프론트엔드 개발 방식에서 벗어나, 각 팀이 자신의 섹션 또는 컴포넌트에 전적으로 책임을 지는 형태임
        - 이러한 구조의 주요 이점은 팀별로 완전히 독립적인 개발 및 배포 파이프라인을 가질 수 있다는 것이 있으며, 각 팀은 자신의 코드 베이스에 대해 완전한 통제권을 갖고, 다른 팀의 작업에 의존하지 않고 독립적으로 업데이트하거나 개선할 수 있고, 이는 개발 프로세스의 유연성을 증가시키고, 빠른 반복과 혁신을 가능하게 함
        - 또한, 분산된 팀 구조는 각 팀이 자신의 기능에 더 집중할 수 있도록 돕고, 팀은 자신의 컴포넌트에 대한 깊은 이해와 전문성을 발전시킬 수 있으며, 이는 전체 애플리케이션의 품질 향상으로 이어질 수 있음. 하지만 이러한 구조는 각 팀 간의 긴밀한 협력과 일관된 기술 스택 및 개발 접근 방식이 필요하다는 점에서, 조직적인 커뮤니케이션과 협력에 대한 추가적인 노력을 요구함

    2. 자율성과 책임감

        - 각 팀이 자신의 마이크로 프론트엔드 컴포넌트나 애플리케이션에 대한 전체적인 책임을 지는데 이는 아키텍처 설계부터 개발, 테스팅, 배포, 유지보수에 이르기까지 모든 과정을 포함하며, 이러한 자율성은 팀이 더 빠른 의사결정을 내리고, 민첩하게 반응할 수 있도록 도움. 예를 들어, 한 팀이 사용자 인터페이스의 특정 부분(예: 결제 시스템)에 대해 책임이 있다면, 이 팀은 해당 기능의 개선, 오류 수정, 업데이트를 자체적으로 관리할 수 있으며, 이는 기능 개선의 속도를 높이고, 다른 팀의 작업에 대한 의존성을 줄임.
        - 자율성은 또한 팀원들에게 더 많은 창의성과 혁신의 기회를 제공하는데 각 팀은 자신들의 컴포넌트에 대한 깊은 이해와 전문 지식을 발전시킬 수 있으며, 이는 전체 프로젝트의 품질과 성능을 향상시킬 수 있으나, 이러한 자율성은 명확한 가이드라인과 규칙이 수립되어야 효과적이며, 모든 팀이 프로젝트의 전반적인 목표와 일관성을 유지하며 작업할 수 있도록, 공통된 코딩 표준, 디자인 가이드 라인, 개발 절차 등을 설정해야함. 이는 개별 팀이 독립적으로 작업하면서도 전체 프로젝트의 통합성을 유지하는 데 필수적임

    3. 협업과 커뮤니케이션

        - 마이크로 프론트엔드 아키텍처 내에서 협업과 커뮤니케이션의 중요성은 매우 크며, 이 아키텍처는 다양한 팀이 각자의 마이크로 애플리케이션 또는 컴포넌트를 독립적으로 개발하도록 설계되어 있지만, 전체 프로젝트의 일관성과 통합을 위해서는 각 팀 간의 긴밀한 협력이 필요함.
        - 예를 들어, 한 팀이 사용자 인터페이스의 검색 기능을 개발하고 다른 팀이 사용자 프로필 관리를 담당한다면, 이 두 기능 간의 상호작용 및 데이터 통합에 대해 명확히 소통 해야하고, 이를 위해 정기적인 미팅, 공동의 기술 스택, 공통된 코딩 및 디자인 가이드라인을 마련하는 것이 중요함. 이렇게 하면 서로 다른 팀이 개발한 컴포넌트들이 원활하게 통합되고, 사용자에게 일관된 경험을 제공할 수 있음. => 이를 위한 별개의 팀이 존재하기도 함.
        - 협업을 촉진하기 위해, 각 팀은 자신의 작업 범위, 진행 상황, 계획 변경 사항 등을 명확히 공유 해야하며, 이는 프로젝트 관리 도구, 공유된 문서, 혹은 정기적인 스크럼 미팅을 통해 이루어질 수 있고, 공통된 문제 해결 방법, 재사용 가능한 컴포넌트, 라이브러리에 대한 액세스를 통해 코드의 일관성을 유지하고, 중복 작업을 방지할 수 있음.
        - 마이크로 프론트엔드 환경에서 협업과 커뮤니케이션은 프로젝트의 성공적인 실행을 위한 핵심 요소이며, 각 팀이 서로의 작업에 대해 잘 이해하고, 공동의 목표를 향해 함께 나아갈 때, 전체 프로젝트의 효율성과 효과성이 크게 향상됨.

    4. 기술 및 프로세스 표준화
        - 기술적 구성 요소의 결정 : 마이크로 프론트엔드의 각 부분은 독립적으로 운영될 수 있어야 하므로, 각 팀은 사용할 기술 스택(예: 프레임워크, 라이브러리, 툴링)을 결정할 때 전체 시스템과의 호환성을 고려해야함. 예를 들어, React, Vue, Angular 등 다양한 프론트엔드 프레임워크 간의 호환성 문제를 해결하는 방안을 마련해야함.
        - 모듈 페더레이션 및 컴포넌트 공유 : 모듈 페더레이션을 사용해 다른 팀이 개발한 컴포넌트를 재사용하는 방법을 구현함. 이는 코드 중복을 줄이고, 일관된 사용자 경험을 제공하는 데 도움이 되며, 공유 컴포넌트 라이브러리를 만들어, 각 팀이 일관된 UX 가이드라인을 따르도록 하는 것도 중요함.
        - 배포 및 통합 전략 : 독립적으로 개발된 마이크로 프론트엔드 컴포넌트들을 어떻게 효율적으로 통합하고 배포할지에 대한 전략을 수립해야하며, CI/CD(Continuous Integration/Continuous Deployment) 파이프라인을 설정하여 각 팀이 자신의 코드를 독립적으로 배포할 수 있도록 하며, 전체 애플리케이션에 대한 통합 테스트를 수행함.

-   팀의 기술적 다양성과 한계
    -   자율과 통제의 trade off 또는 balance가 필요함.
        -   자율 : 분산된 팀 구조에서 자율성과 책임감 등
        -   통제 : 협업과 커뮤니케이션, 기술 및 프로세스 표준화 등
    -   너무 자율적인 팀
        -   기술적으로 너무 자율적인 팀은 통합된 애플리케이션을 만들 때 여러가지 문제가 발생할 수 있음
        -   서로 다른 기술 스택을 사용하는 팀 간의 협업과 통합은 복잡해질 수 있으며, 전체 프로젝트의 일관된 사용자 경험과 스타일 가이드를 유지하는 데 어려움을 겪을 수 있음
        -   또한, 각 팀의 기술 선택이 프로젝트의 전체적인 성능, 유지보수성, 확장성에 영향을 미칠 수 있으므로, 이러한 요소들을 고려한 기술 선택이 필요함
        -   예를 들어 어떤 팀은 Vue 를 사용하고, 어떤 팀은 React 를 사용하는 경우, 일관된 사용자 경험을 위한 공유 컴포넌트 라이브러리를 함께 사용할 수 없으며, 이슈가 발생할 경우 두가지 모두 유지 보수해야함
        -   사실 이러한 문제는 React 를 사용하는데 상태 관리에 사용하는 라이브러리가 다른 것도 같은 맥락이고, 결국엔 어떤 선에서 결정을 해야 하는 문제임
    -   너무 통제적인 팀
        -   마이크로 프론트엔드 아키텍처에서 팀 운영이 지나치게 통제되는 경우, 여러 단점이 발생할 수 있고, 이러한 환경에서는 각 팀의 자율성과 창의성이 제한될 수 있으며, 이는 전반적인 프로젝트의 혁신과 효율성에 부정적인 영향을 미칠 수 있음
        -   예를 들어, 모든 마이크로 프론트엔드 팀이 동일한 기술 스택, 도구, 개발 절차를 엄격하게 따라야 한다고 가정해보면, 이 경우, 특정 팀이 자신의 기능에 더 적합한 새로운 기술이나 도구를 실험하거나 채택하는 것이 어려워질 수 있고, 결과적으로, 팀은 기존의 방식에 얽매여 혁신적인 솔루션을 탐색하는 기회를 잃게 됨
        -   또한, 지나친 통제는 팀원들의 동기 부여와 만족도를 저하시킬 수 있으며, 팀원들이 자신의 의견이나 창의적인 아이디어를 자유롭게 제시하고 시도할 수 없다면, 이는 직무 만족도 감소로 이어질 수 있으며, 장기적으로는 인재 유출의 원인이 될 수 있음
    -   균형을 이루는 법
        -   공통 기술 스택을 최소한으로 하는것임. 예를 들어 React 를 사용하는 것만 정하고 나머지는 공통으로 사용하지 않도록 하고, Redux 와 같은 상태 관리 라이브러리를 공통으로 하는 경우, 이 라이브러리에 얽매여 다른 기술적인 시도를 하기가 매우 어려워짐
        -   전체 코딩 가이드라인, 디자인 라이브러리, 인프라 레벨의 패키지를 관리하는 팀 혹은 개인을 두는것이 좋으며, 이러한 종류의 업무는 모든 구성원들이 자유롭게 기여할 수 있도록 운영함
        -   각 마이크로 프론트엔드 팀들이 정기적인 기술 교류를 가질 수 있도록 긍적적인 문화를 형성하는것도 좋음(스터디, 기술 교류 컨퍼런스 등). 이러한 부분들은 각 팀들이 고립되지 않도록 하는 중요한 역할을 가지고 있음
...
-   브랜치 전략과 배포 방식
    ...
    -   모노레포와 워크스페이스 특징
        ...
        -   모노레포(Monorepo)는 여러 프로젝트의 코드를 단일 저장소에서 관리하는 접근 방식이고, 이 구조에서 효과적인 브랜치 전략과 배포 방식은 프로젝트의 복잡성을 관리하고, 팀 간의 협업을 원활하게 하는 데 중요함
        -   각 워크스페이스는 그 역할에 따라 작업의 속도와 변경 폭, 배포 주기가 매우 다름
            -   빌드 타임에 공유되는 인프라 레벨의 패키지들은 느리고 신중하게 움직이고, 전체 코드베이스에 큰 영향을 끼칠 가능성이 있음
            -   각 마이크로 앱은 다른 영역으로 크게 영향을 주지 않으며, 앱 영역 안에서 활발하게 작업이 진행됨

    -   마이크로 앱 브랜치
        -   마이크로 앱은 다음 작업 주기가 시작되면 별도의 마이크로 앱 브랜치를 생성하고 그 브랜치를 기준으로 피쳐 브랜치를 생성하여, 마이크로 앱의 작업자들이 활발하게 작업하며 코드 리뷰와 PR 을 통해 마이크로 앱 브랜치 로 병합함
        -   이때 만약에 함께 수정될 인프라 레벨 패키지가 있다면, 반드시 전체 마이크로 앱 오너들에게 코드 리뷰를 받아 영향도와 배포 순서 혹은 전략을 결정해야함
            -   예를 들어 영향이 없다면 스코프 처리만 하고 다른 마이크로 앱들은 배포하지 않음
            -   하지만 영향이 크다면 함께 배포를 하고 순서를 결정함
    -   변경 감지 기반 배포
        -   대규모 프로젝트에서는 배포 프로세스를 효율화하기 위해 변경된 부분만을 감지하여 배포하는 '변경감지 기반 배포' 방식을 적용할 수 있으며, 이는 불필요한 빌드와 배포를 줄여 전체적인 개발 속도를 향상시킴
        -   하지만 너무 잦은 배포를 발생시킬수 있고, 패키지 변경에 따른 배포 범위를 결정할 때, 단순 변경 만으로 판단할 수 없는 부분이 있어 주의를 요구함

-   로컬 개발 환경과 프로덕션 환경
    -   로컬에서 모든 마이크로 앱을 전부 개발할 때
        -   Shell 과 모든 마이크로 앱을 동시에 로컬에서 실행하는 방식
        -   로컬 머신의 리소스는 유한하고 마이크로 앱과 프레그먼트들은 무한히 확장할 가능성이 있기 때문에 어떤 선을 넘어서면 로컬에서 모든 앱을 동시에 띄우고 개발하기에 힘들고 느려짐
        -   물론 전체앱을 사용해볼 수 있다는 장점이 있지만, 마이크로 앱 개발자로서 사용하는 부분은 자신의 마이크로 앱과 관련 프레그먼트들 임
        -   전체 앱이 비대해지면 발생하는 문제가 개발 경험이 좋지 않는 것인데 이런 방식은 원래의 문제를 유지한 채 개발하는 것이기 때문에 지양해야함
    -   나의 마이크로 앱만을 로컬에서 개발할 때
        -   그렇다면 나의 마이크로 앱만을 개발하는 것이 올바른 방식이고 이를 위해서는 크게 두가지 방향이 있음
            -   첫번째는 나의 마이크로 앱과 마이크로 앱을 띄우기 위해 필요한 의존 앱을 모두 띄우는 방식
            -   두번째는 나의 마이크로 앱만을 띄워서 개발이 가능하도록 의존 앱의 기능을 마이크로 앱에 탑재하는 방식이고, 실제로 이는 프레그먼트 개발의 경우 매우 유용하고 필요한 기능
    -   하이브리드 방식
        -   마이크로 앱 개발을 위한 서버 환경을 꾸미고 나의 개발 환경을 사용하도록 연결하는 방법이 있으며, 이것이 사실 연결했을 때 가장 확실한 진짜 테스트가 가능한 환경임
        -   하지만, 보안에 이슈가 있으며, 각 마이크로 앱 개발을 위한 서버 환경을 여러벌 만들어서 리소스가 많이 필요하다는 단점이 있음

### 장애 대응

-   단일 장애 지점을 피했다면, 장애의 범위를 줄여나가기 위한 전략과 한계
    -   마이크로 프론트엔드의 목표
        -   단일 장애 지점을 피하는 것
        -   전체 복잡도를 관리가 가능한 수준으로 쪼개어 개발하기
    -   마이크로 프론트엔드의 한계
        -   무조건 쪼개는 것이 답이 아님
        -   관리가 가능한 수준으로 나누는 것이 중요
        -   결국 분리한 만큼 서버가 별도로 필요
        -   분리해서 배포하기 위해 중복 코드를 두려워하지 않아야함
...
-   마이크로 프론트엔드 아키텍처의 필요성

    -   조직간 업무 결합도 증가
        -   복잡해지는 서비스, 조직구조에 따라 조직간 업무 결합도 증가의 문제가 발생
    -   중복 개발의 문제
        -   동일하거나, 유사한 정보에 대한 표출을 서로 다른 부서에서 중복적으로 진행하여, 조직의 효율성을 저해
...
-   MFA 레거시 도입의 장벽들
    -   CSS polution
        -   인젝션된 앱이 기존 앱 디자인에 영향을 미칠 수 있음
    -   Config 설정
        -   환경 설정이 어렵다면, 마이크로 앱 단위로 좋은 기능을 만들어도 도입하기 어려움
